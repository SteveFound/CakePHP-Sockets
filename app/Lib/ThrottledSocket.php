...
class ThrottledSocket extends BaseSocket implements ISocket {
     ...
     * once a second. We achieve this by writing the time the last request
     * will be sent to a file. When the time from the file is read, it will
     * either be in the past in which case we can send our request immediately
     * and set the new time to now, or it will be in the future meaning that 
     * other requests are already queued. In this case we add one second to 
     * that time and write it back. The difference between the updated time and
     * now is the number of seconds we must wait before we can send our request.
    ...
    private function throttle() {
        ...
        if (flock($fp, LOCK_EX)) {
            // re-read the current time as we may have waited for the lock
            $curtime = time();
        	// Get the time the last request was or will be sent.
            $nbr = fread($fp, filesize($this->_filename));
            $filetime = intval(trim($nbr));
            // If the last request was in the past, write our time to the file
            // and we can make our request immediately
            if ($curtime > $filetime) {
                $filetime = $curtime;
            } else {
            	// Otherwise our request can be sent 1 second after the last one
                $filetime++;
            }
            // Write the request time back to the file
            rewind($fp);
            ftruncate($fp, 0);
        	...
        	fclose($fp);
        	throw new RuntimeException("Unable to lock throttle file");
        }
        // return how many seconds we have to wait before we can send
